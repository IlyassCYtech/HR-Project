...
public class CongeAbsenceServlet extends HttpServlet {
    ...
    private void afficherFormulaireModification(HttpServletRequest request, HttpServletResponse response) 
        ...
        boolean isOwnConge = employeIdSession != null && 
            ...
            employeIdSession.equals(conge.getEmploye().getId());
        
        // Seul le propriétaire ou RH/Admin peuvent accéder au formulaire de modification
        if (!isRHorAdmin && !isOwnConge) {
            logger.warn("Tentative d'accès non autorisé au formulaire de modification du congé {} par l'utilisateur {}", 
                id, employeIdSession);
    ...
    private void modifierConge(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            if (!isRHorAdmin && !isOwnConge) {
                logger.warn("Tentative de modification non autorisée du congé {} par utilisateur {} (employeId: {})", 
                    id, utilisateur.getUsername(), employeIdSession);
                request.setAttribute("error", "Vous n'êtes pas autorisé à modifier ce congé");
                response.sendRedirect(request.getContextPath() + "/app/conges-absences");
                return;
            }
            ...
            logger.info("Congé {} modifié avec succès par {}", id, utilisateur.getUsername());
    ...
    private void supprimerConge(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            // *** SÉCURITÉ: Vérifier les droits de suppression ***
            HttpSession session = request.getSession();
            Utilisateur utilisateur = (Utilisateur) session.getAttribute("utilisateur");
            Long employeIdSession = (Long) session.getAttribute("employeId");
            ...
            if (!isRHorAdmin && !isOwnConge) {
                logger.warn("Tentative de suppression non autorisée du congé {} par utilisateur {} (employeId: {})", 
                    id, utilisateur.getUsername(), employeIdSession);
                request.setAttribute("error", "Vous n'êtes pas autorisé à supprimer ce congé");
                response.sendRedirect(request.getContextPath() + "/app/conges-absences");
                return;
            }
            ...
            logger.info("Congé {} supprimé avec succès par {}", id, utilisateur.getUsername());
