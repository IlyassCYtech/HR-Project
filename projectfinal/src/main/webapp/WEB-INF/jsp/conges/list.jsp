        ...
        <div style="padding: 24px; display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px;">
            <div>
		        ...
		        <c:if test="${utilisateur.role eq 'ADMIN' or utilisateur.role eq 'RH'}">
		            ...
		            <c:forEach var="emp" items="${employes}">
		                <option value="${emp.id}"
		                        <c:if test="${currentEmployeId == emp.id.toString()}">selected</c:if>>
		                    ${emp.prenom} ${emp.nom}
		                </option>
		            </c:forEach>
		        </c:if>
		        <c:if test="${utilisateur.role ne 'ADMIN' and utilisateur.role ne 'RH'}">
            ...
            <div>
                ...
                <select id="statut" name="statut" class="form-control">
                    ...
                    <option value="REFUSE" ${param.statut == 'REFUSE' ? 'selected' : ''}>Refusé</option>
                        ...
                        <tr>
                            ...
                            <td style="text-align: center;">
                                <div style="display: inline-flex; gap: 6px; align-items: center;">
                                    ...
                                    <c:if test="${conge.statut.name() == 'EN_ATTENTE' and (utilisateur.role eq 'ADMIN' or utilisateur.role eq 'RH')}">
                                    ...
                                    <c:if test="${conge.statut.name() == 'REFUSE' and (utilisateur.role eq 'ADMIN' or utilisateur.role eq 'RH')}">
                                        <form action="${pageContext.request.contextPath}/app/conges-absences" method="post" style="display: inline; margin: 0;">
                                            <input type="hidden" name="action" value="approve">
                                            <input type="hidden" name="id" value="${conge.id}">
                                            <button type="submit" 
