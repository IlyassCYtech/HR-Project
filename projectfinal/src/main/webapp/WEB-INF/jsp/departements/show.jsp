...
<%-- Déterminer si l'utilisateur consulte son propre département --%>
<c:set var="isRHorAdmin" value="${utilisateur.role eq 'ADMIN' or utilisateur.role eq 'RH'}" />
<c:set var="isOwnDept" value="${not empty employeConnecte and not empty employeConnecte.departement and employeConnecte.departement.id == departement.id}" />
<c:set var="isChefDeptOwnDept" value="${utilisateur.role eq 'CHEF_DEPT' and isOwnDept}" />
<c:set var="canViewConfidential" value="${isRHorAdmin or isOwnDept}" />
<c:set var="canManageMembers" value="${isRHorAdmin or isChefDeptOwnDept}" />
...
<div class="stats-grid" style="grid-template-columns: repeat(3, 1fr); margin-bottom: 32px;">
    ...
    <div class="stat-card">
        ...
        <h4>
            <c:choose>
            	<c:when test="${not canViewConfidential}">
        ...
        <p>
            <c:choose>
                <c:when test="${not canViewConfidential}">
                    <span style="color: #999;">Confidentiel</span>
                </c:when>
                <c:otherwise>Budget annuel</c:otherwise>
            </c:choose>
        </p>
    </div>
    <div class="stat-card">
...
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 24px;">
                ...
                <c:choose>
                    <c:when test="${not empty departement.chefDepartement}">
                        <div style="display: flex; align-items: center; gap: 12px; margin-top: 12px;">
                            <div class="avatar">
							    ${departement.chefDepartement.initiales}
							</div>
                            <div>
    ...
    <div class="card">
        ...
        <div style="padding: 32px;">
            <div style="margin-bottom: 24px;">
                ...
                <c:choose>
                    <c:when test="${utilisateur.role eq 'EMPLOYE' or utilisateur.role eq 'CHEF_PROJET'}">
                        <p style="font-size: 16px; color: #999999; font-style: italic; margin-top: 8px;">
                    ...
                    <c:otherwise>
                        <p style="font-size: 28px; color: #999999; font-weight: 600; margin-top: 8px;">
                            Non défini
                        </p>
                    </c:otherwise>
                </c:choose>
            ...
            <c:if test="${not empty departement.budget && departement.budget > 0 and canViewConfidential}">
                        ...
                        <tr>
                            ...
                            <td style="text-align: center;">
                                <div style="display: flex; gap: 8px; justify-content: center;">
                                    ...
                                    <%-- 
                                        Bouton Retirer du département :
                                        - ADMIN et RH : peuvent retirer n'importe qui
                                        - CHEF_DEPT : peut retirer les membres SAUF lui-même, ET SEULEMENT dans SON département
                                    --%>
                                    <c:choose>
                                        <%-- ADMIN et RH : accès complet --%>
                                        <c:when test="${isRHorAdmin}">
                                            <a href="${pageContext.request.contextPath}/app/departements?action=retirerEmploye&departementId=${departement.id}&employeId=${emp.id}" 
                                               class="btn btn-danger btn-sm" 
                                               title="Retirer du département"
                                               onclick="return confirm('⚠️ Voulez-vous vraiment retirer ${emp.prenom} ${emp.nom} du département ${departement.nom} ?');">
                                                <i class="fas fa-user-minus"></i>
                                            </a>
                                        </c:when>
                                        <%-- CHEF_DEPT : peut retirer les membres sauf lui-même, ET UNIQUEMENT dans son propre département --%>
                                        <c:when test="${isChefDeptOwnDept and 
                                                        not empty employeConnecte and 
                                                        employeConnecte.id != emp.id}">
                                            <a href="${pageContext.request.contextPath}/app/departements?action=retirerEmploye&departementId=${departement.id}&employeId=${emp.id}" 
                                               class="btn btn-danger btn-sm" 
                                               title="Retirer du département"
                                               onclick="return confirm('⚠️ Voulez-vous vraiment retirer ${emp.prenom} ${emp.nom} du département ${departement.nom} ?');">
