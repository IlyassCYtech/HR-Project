...
CREATE TABLE employes (
    ...
    statut ENUM('ACTIF', 'EN_CONGE', 'SUSPENDU', 'DEMISSIONNAIRE', 'LICENCIE', 'RETRAITE') DEFAULT 'ACTIF',
    departement_id INT,
    manager_id INT,
...
-- ============================================================================
-- TRIGGER : Synchronisation automatique email employé -> utilisateur
-- ============================================================================
-- Ce trigger met à jour automatiquement l'email dans la table utilisateurs
-- quand l'email d'un employé est modifié. Cela garantit que les deux tables
-- restent synchronisées sans intervention manuelle.
-- ============================================================================

DROP TRIGGER IF EXISTS trg_employes_after_update_email;

DELIMITER $$

CREATE TRIGGER trg_employes_after_update_email
AFTER UPDATE ON employes
FOR EACH ROW
BEGIN
    -- Vérifie si l'email a changé (opérateur <=> gère les NULL)
    IF NOT (OLD.email <=> NEW.email) THEN
        -- Met à jour l'email de l'utilisateur lié (s'il existe)
        UPDATE utilisateurs 
        SET email = NEW.email 
...
-- Mot de passe rh: rh123
-- Mot de passe manager: manager123
-- Mot de passe password123 pour tous les autres
INSERT INTO utilisateurs (username, password_hash, email, role, employe_id, statut, tentatives_connexion) VALUES
-- Comptes administratifs RELIÉS aux employés (avec les MÊMES emails que les employés)
('admin', '$2b$10$gjg6NDpKJzbpagTbfIE3x.FcO39QwiaF/QUG8g7UnT9or6fO7wztO', 'jean.martin@entreprise.com', 'ADMIN', 1, 'ACTIF', 0),  -- Jean Martin
('rh', '$2b$10$O2WF4bpZVvvA1U3izKtNxu.CmgxxnWcBn9gqtwWiNKh/svclJmkri', 'marie.dubois@entreprise.com', 'RH', 2, 'ACTIF', 0),  -- Marie Dubois
('manager', '$2b$10$hQRDdH1q/GBd1rfq2sUql.CdXQ4R3O.iZVneBEe4gQfM9otuZE6wm', 'sophie.bernard@entreprise.com', 'CHEF_DEPT', 4, 'ACTIF', 0),  -- Sophie Bernard

-- Comptes pour les autres employés
('pierre.leroy', '$2b$10$7s0xXOsX8DMglsQmfWMYh.ODGR/nIW9j02vrG.u9F7bx/eY7d7NsC', 'pierre.leroy@entreprise.com', 'CHEF_PROJET', 3, 'ACTIF', 0),
('paul.rousseau', '$2b$10$7s0xXOsX8DMglsQmfWMYh.ODGR/nIW9j02vrG.u9F7bx/eY7d7NsC', 'paul.rousseau@entreprise.com', 'CHEF_DEPT', 5, 'ACTIF', 0),
('claire.durand', '$2b$10$7s0xXOsX8DMglsQmfWMYh.ODGR/nIW9j02vrG.u9F7bx/eY7d7NsC', 'claire.durand@entreprise.com', 'EMPLOYE', 6, 'ACTIF', 0),
('luc.moreau', '$2b$10$7s0xXOsX8DMglsQmfWMYh.ODGR/nIW9j02vrG.u9F7bx/eY7d7NsC', 'luc.moreau@entreprise.com', 'EMPLOYE', 7, 'ACTIF', 0),
('anne.girard', '$2b$10$7s0xXOsX8DMglsQmfWMYh.ODGR/nIW9j02vrG.u9F7bx/eY7d7NsC', 'anne.girard@entreprise.com', 'EMPLOYE', 8, 'ACTIF', 0),
('marc.roux', '$2b$10$7s0xXOsX8DMglsQmfWMYh.ODGR/nIW9j02vrG.u9F7bx/eY7d7NsC', 'marc.roux@entreprise.com', 'EMPLOYE', 9, 'ACTIF', 0),
('julie.blanc', '$2b$10$7s0xXOsX8DMglsQmfWMYh.ODGR/nIW9j02vrG.u9F7bx/eY7d7NsC', 'julie.blanc@entreprise.com', 'EMPLOYE', 10, 'ACTIF', 0),
('thomas.faure', '$2b$10$7s0xXOsX8DMglsQmfWMYh.ODGR/nIW9j02vrG.u9F7bx/eY7d7NsC', 'thomas.faure@entreprise.com', 'EMPLOYE', 11, 'ACTIF', 0),
('emma.laurent', '$2b$10$7s0xXOsX8DMglsQmfWMYh.ODGR/nIW9j02vrG.u9F7bx/eY7d7NsC', 'emma.laurent@entreprise.com', 'EMPLOYE', 12, 'ACTIF', 0),
('hugo.michel', '$2b$10$7s0xXOsX8DMglsQmfWMYh.ODGR/nIW9j02vrG.u9F7bx/eY7d7NsC', 'hugo.michel@entreprise.com', 'EMPLOYE', 13, 'ACTIF', 0);
...
COMPTES ADMINISTRATIFS (sans employé lié) :
- admin / admin123           → Administrateur système
- rh / rh123                 → Ressources Humaines
- manager / manager123       → Chef de département

COMPTES EMPLOYÉS (tous avec password123) :
- jean.martin / password123        → Directeur Général (ADMIN)
- marie.dubois / password123       → Directrice RH (RH)
- pierre.leroy / password123       → Chef de projet senior (CHEF_PROJET)
- sophie.bernard / password123     → Responsable Finance (CHEF_DEPT)
- paul.rousseau / password123      → Chef Marketing (CHEF_DEPT)
- claire.durand / password123      → Développeur Senior (EMPLOYE)
- luc.moreau / password123         → Analyste RH (EMPLOYE)
- anne.girard / password123        → Comptable Senior (EMPLOYE)
- marc.roux / password123          → Développeur Junior (EMPLOYE)
- julie.blanc / password123        → Assistante RH (EMPLOYE)
- thomas.faure / password123       → Analyste Marketing (EMPLOYE)
- emma.laurent / password123       → Stagiaire Développement (EMPLOYE)
- hugo.michel / password123        → Stagiaire Marketing (EMPLOYE)
...
- 4 projets insérés
- 16 utilisateurs insérés (3 admin + 13 employés)
