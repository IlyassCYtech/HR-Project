...
public class EmployeViewController {
    ...
    public String listEmployes(
        ...
        // Récupérer tous les employés et appliquer TOUS les filtres en AND (pas OR)
        List<Employe> employes = employeService.findAll();

        employes = employes.stream()
                .filter(e -> {
                    // Filtre par recherche (nom, prénom, matricule) - AND
                    if (search != null && !search.trim().isEmpty()) {
                        String searchLower = search.toLowerCase();
                        boolean matchSearch = e.getNom().toLowerCase().contains(searchLower) ||
                                e.getPrenom().toLowerCase().contains(searchLower) ||
                                e.getMatricule().toLowerCase().contains(searchLower);
                        if (!matchSearch)
                            return false;
                    }

                    // Filtre par département - AND
                    if (departement != null) {
                        boolean matchDept = e.getDepartement() != null
                                && e.getDepartement().getId().equals(departement);
                        if (!matchDept)
                            return false;
                    }

                    // Filtre par poste - AND
                    if (poste != null && !poste.trim().isEmpty()) {
                        boolean matchPoste = e.getPoste() != null && e.getPoste().equalsIgnoreCase(poste);
                        if (!matchPoste)
                            return false;
                    }

                    // Filtre par statut - AND
                    if (statut != null && !statut.trim().isEmpty()) {
                        boolean matchStatut = e.getStatut() != null && e.getStatut().name().equalsIgnoreCase(statut);
                        if (!matchStatut)
                            return false;
                    }

                    return true; // Tous les filtres passent
                })
                .toList();
        ...
        // Ajouter l'ID de l'employé connecté pour permettre la modification de son
        // propre profil
        org.springframework.security.core.Authentication authentication = org.springframework.security.core.context.SecurityContextHolder
                .getContext().getAuthentication();
        if (authentication != null && authentication.getName() != null) {
            Optional<Utilisateur> utilisateur = utilisateurService.getUtilisateurByUsername(authentication.getName());
            if (utilisateur.isPresent() && utilisateur.get().getEmploye() != null) {
                model.addAttribute("employeConnecteId", utilisateur.get().getEmploye().getId());
            }
        }
    ...
    public String showCreateForm(Model model, RedirectAttributes redirectAttributes) {
        ...
        if (utilisateurOpt.isPresent()) {
            ...
            com.gestionrh.projetfinalspringboot.model.enums.Role userRole = utilisateur.getRole();

            // Seuls ADMIN et RH peuvent créer des employés
            if (userRole != com.gestionrh.projetfinalspringboot.model.enums.Role.ADMIN &&
                    userRole != com.gestionrh.projetfinalspringboot.model.enums.Role.RH) {
                redirectAttributes.addFlashAttribute("error", "Vous n'avez pas les droits pour créer un employé");
        ...
        model.addAttribute("canViewEmploye", true);
        model.addAttribute("isAdminOrRH", true);
        model.addAttribute("isSelfEdit", false);
        model.addAttribute("hasUtilisateur", false);  // Nouvel employé n'a pas encore d'utilisateur
        model.addAttribute("activePage", "employes");
    ...
    public String showEditForm(@PathVariable Long id, Model model, RedirectAttributes redirectAttributes) {
        ...
        // Vérifier les permissions pour l'accès au formulaire
        org.springframework.security.core.Authentication authentication = org.springframework.security.core.context.SecurityContextHolder
                .getContext().getAuthentication();
        String username = authentication.getName();
        Optional<Utilisateur> utilisateurOpt = utilisateurService.getUtilisateurByUsername(username);
        ...
        if (utilisateurOpt.isPresent()) {
            ...
            // Si ce n'est pas ADMIN ou RH, vérifier que c'est son propre profil
            if (userRole != com.gestionrh.projetfinalspringboot.model.enums.Role.ADMIN &&
                    userRole != com.gestionrh.projetfinalspringboot.model.enums.Role.RH) {
        ...
        // Ajouter les attributs pour la vue Thymeleaf
        boolean isAdminOrRH = utilisateurOpt.isPresent() &&
                (utilisateurOpt.get().getRole() == com.gestionrh.projetfinalspringboot.model.enums.Role.ADMIN ||
                        utilisateurOpt.get().getRole() == com.gestionrh.projetfinalspringboot.model.enums.Role.RH);

        boolean isSelf = utilisateurOpt.isPresent() &&
                utilisateurOpt.get().getEmploye() != null &&
                utilisateurOpt.get().getEmploye().getId().equals(id);

        // Récupérer l'utilisateur associé à cet employé (s'il existe)
        Optional<Utilisateur> employeUtilisateur = utilisateurService.findByEmployeId(id);
        model.addAttribute("employeUtilisateur", employeUtilisateur.orElse(null));
        model.addAttribute("hasUtilisateur", employeUtilisateur.isPresent());

        model.addAttribute("canViewEmploye", isAdminOrRH || isSelf);
        model.addAttribute("isAdminOrRH", isAdminOrRH);
        model.addAttribute("isSelfEdit", isSelf && !isAdminOrRH);
        model.addAttribute("activePage", "employes");
        return "employes/form";
    }
    ...
    public String createEmploye(
        ...
        try {
            ...
            if (managerId != null && managerId > 0) {
                employeService.findById(managerId)
    ...
    public String updateEmploye(
            ...
            @RequestParam(required = false) Long departementId,
            @RequestParam(required = false) Long managerId,
            @RequestParam(required = false) String utilisateurRole,
            RedirectAttributes redirectAttributes) {
        try {
            ...
            boolean isAdminOrRH = false;
            if (utilisateurOpt.isPresent()) {
                Utilisateur utilisateur = utilisateurOpt.get();
                com.gestionrh.projetfinalspringboot.model.enums.Role userRole = utilisateur.getRole();

                isAdminOrRH = (userRole == com.gestionrh.projetfinalspringboot.model.enums.Role.ADMIN ||
                               userRole == com.gestionrh.projetfinalspringboot.model.enums.Role.RH);
            ...
            existing.setDateFin(employe.getDateFin()); // Date fin de contrat
            ...
            // *** MISE À JOUR DU RÔLE : Seuls ADMIN et RH peuvent modifier le rôle ***
            // On le fait AVANT de sauvegarder l'employé pour éviter les problèmes de transaction
            String roleUpdateMessage = "";
            
            if (isAdminOrRH && utilisateurRole != null && !utilisateurRole.isEmpty()) {
                Optional<Utilisateur> employeUtilisateurOpt = utilisateurService.findByEmployeId(id);
                
                if (employeUtilisateurOpt.isPresent()) {
                    try {
                        // Recharger l'utilisateur complet avec toutes ses données
                        // findByEmployeId ne charge pas tous les champs à cause du lazy loading
                        Long utilisateurId = employeUtilisateurOpt.get().getId();
                        Utilisateur employeUtilisateur = utilisateurService.getUtilisateurById(utilisateurId)
                            .orElseThrow(() -> new RuntimeException("Utilisateur introuvable"));
                        
                        Role newRole = Role.valueOf(utilisateurRole);
                        employeUtilisateur.setRole(newRole);
                        utilisateurService.save(employeUtilisateur);
                        roleUpdateMessage = " Rôle modifié : " + newRole.name() + ".";
                    } catch (IllegalArgumentException e) {
                        redirectAttributes.addFlashAttribute("error", "Rôle invalide : " + utilisateurRole);
                        return "redirect:/employes/form/" + id;
                    }
                }
            }
            ...
            redirectAttributes.addFlashAttribute("message",
                    "Employé " + updated.getPrenom() + " " + updated.getNom() + " mis à jour avec succès." + roleUpdateMessage);
    ...
    public String showEmploye(@PathVariable Long id, Model model, RedirectAttributes redirectAttributes) {
        ...
        // Ajout de la logique de sécurité pour afficher le bouton "Modifier"
        org.springframework.security.core.Authentication authentication = org.springframework.security.core.context.SecurityContextHolder
                .getContext().getAuthentication();
        String username = authentication.getName();
        Optional<Utilisateur> utilisateurOpt = utilisateurService.getUtilisateurByUsername(username);
        ...
        if (utilisateurOpt.isPresent()) {
            ...
            if (utilisateur.getEmploye() != null) {
                isSelf = utilisateur.getEmploye().getId().equals(id);
            }
        }
    ...
    public String generateCredentials(@RequestParam Long employeId, HttpSession session,
            RedirectAttributes redirectAttributes) {
        try {
            ...
            Utilisateur utilisateur = Utilisateur.builder()
                    ...
                    .role(Role.EMPLOYE)
                    .employe(employe) // LIER L'EMPLOYÉ AU COMPTE UTILISATEUR
    ...
    public String resetPassword(@RequestParam Long employeId, HttpSession session,
            RedirectAttributes redirectAttributes) {
        try {
            Optional<Employe> employeOpt = employeService.findById(employeId);
            if (!employeOpt.isPresent()) {
    ...
    public String deleteEmploye(@PathVariable Long id, Authentication authentication, RedirectAttributes redirectAttributes) {
        try {
            ...
            if (employe.isPresent()) {
                ...
                redirectAttributes.addFlashAttribute("message",
                        "Employé " + employe.get().getPrenom() + " " + employe.get().getNom()
