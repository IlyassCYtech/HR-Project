...
public class FichePaieViewController {
    ...
    public String listFichesPaie(Model model, 
        ...
        // *** RESTRICTION : Si l'utilisateur n'est ni RH ni ADMIN, ne montrer que ses propres fiches ***
        if (userRole != Role.RH && userRole != Role.ADMIN) {
            ...
            // Forcer le filtre sur l'employé de l'utilisateur connecté
            fichesPaie = fichePaieService.findByEmployeId(utilisateur.getEmploye().getId());
        } else {
            // RH et ADMIN peuvent voir toutes les fiches avec filtres
            ...
            if (mois != null && !mois.isEmpty()) {
                ...
                if (parts.length == 2) {
                    ...
                    fichesPaie = fichePaieService.findAll().stream()
                        .filter(f -> f.getMois() == moisNum && f.getAnnee() == annee)
                        .toList();
                } else {
            ...
            if (employeId != null) {
                fichesPaie = fichesPaie.stream()
                    .filter(f -> f.getEmploye() != null && f.getEmploye().getId().equals(employeId))
                    .toList();
            }
        }
        ...
        if (departementId != null) {
            fichesPaie = fichesPaie.stream()
                .filter(f -> f.getEmploye() != null && f.getEmploye().getDepartement() != null 
                    && f.getEmploye().getDepartement().getId().equals(departementId))
                .toList();
        }
        ...
        model.addAttribute("departements", employeService.findAll().stream()
            ...
            .distinct()
            .toList());
        model.addAttribute("page", "fiches-paie");
    ...
    public String generateAllFichesPaie(@RequestParam int mois, @RequestParam int annee, RedirectAttributes redirectAttributes) {
        try {
            int generated = fichePaieService.generateAllFichesPaie(employeService.findAll(), mois, annee);
            redirectAttributes.addFlashAttribute("success", generated + " fiches de paie générées pour " + getMoisNom(mois) + " " + annee);
        } catch (Exception e) {
    ...
    public String listFichesPaieByEmploye(@PathVariable Long employeId, Model model, RedirectAttributes redirectAttributes) {
        var employe = employeService.findById(employeId);
        if (employe.isEmpty()) {
        ...
        fichePaieService.findById(id).ifPresent(fiche -> {
            ...
            if (utilisateur.getRole() != Role.RH && utilisateur.getRole() != Role.ADMIN) {
                if (utilisateur.getEmploye() == null || 
                    ...
                    throw new RuntimeException("Accès refusé : vous ne pouvez télécharger que vos propres fiches de paie");
                }
            }
    ...
    public String updateFichePaie(@PathVariable Long id, @ModelAttribute FichePaie fichePaie, RedirectAttributes redirectAttributes) {
        try {
            // VALIDATION : Le salaire NET ne peut pas être négatif
            if (fichePaie.getNetAPayer() != null && fichePaie.getNetAPayer().compareTo(java.math.BigDecimal.ZERO) <= 0) {
                redirectAttributes.addFlashAttribute("error", 
                    "Le salaire NET à payer ne peut pas être négatif ou nul (" + fichePaie.getNetAPayer() + " €)");
                return "redirect:/fiches-paie/form/" + id;
            }
            
            // Vérification cohérence : salaire base > cotisations + impôts (si tous les champs sont présents)
            if (fichePaie.getSalaireBase() != null && fichePaie.getCotisationsSociales() != null) {
                ...
                if (fichePaie.getSalaireBase().compareTo(total) < 0) {
                    redirectAttributes.addFlashAttribute("error", 
                        "Le salaire de base (" + fichePaie.getSalaireBase() + " €) est insuffisant pour couvrir les charges (" + total + " €)");
                    return "redirect:/fiches-paie/form/" + id;
                }
            }
    ...
    public String showFichePaie(@PathVariable Long id, Model model, RedirectAttributes redirectAttributes) {
        ...
        if (utilisateur.getRole() != Role.RH && utilisateur.getRole() != Role.ADMIN) {
            if (utilisateur.getEmploye() == null || 
                ...
                redirectAttributes.addFlashAttribute("error", "Accès refusé : vous ne pouvez consulter que vos propres fiches de paie");
                return "redirect:/fiches-paie/list";
            }
        }
    ...
    public void downloadAllFichesZip(HttpServletResponse response, 
        ...
        try {
            ...
            if (search != null && !search.isEmpty()) {
                ...
                fiches = fiches.stream()
                    .filter(f -> {
                        String nomComplet = (f.getEmploye().getPrenom() + " " + f.getEmploye().getNom()).toLowerCase();
    ...
    /**
     * Télécharger toutes les fiches de paie filtrées dans un fichier ZIP (méthode POST)
     * Applique les mêmes filtres que l'affichage de la liste
     */
    ...
    /**
     * Télécharger toutes les fiches de paie d'un employé dans un fichier ZIP
     */
    @GetMapping("/download-employee-zip/{employeId}")
    public void downloadEmployeeFichesZip(@PathVariable Long employeId, HttpServletResponse response) {
        try {
            var employe = employeService.findById(employeId)
                .orElseThrow(() -> new RuntimeException("Employé introuvable"));
            List<FichePaie> fiches = fichePaieService.findByEmployeId(employeId);
