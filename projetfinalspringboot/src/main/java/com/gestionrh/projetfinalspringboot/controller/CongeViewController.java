...
public class CongeViewController {
    ...
    @GetMapping("/show/{id}")
    public String showConge(@PathVariable Long id, Model model, RedirectAttributes redirectAttributes, Authentication authentication) {
        Optional<CongeAbsence> conge = congeService.findById(id);
        if (conge.isEmpty()) {
            redirectAttributes.addFlashAttribute("error", "Congé introuvable");
            return "redirect:/conges/list";
        ...
        // *** SÉCURITÉ : Vérifier si l'utilisateur peut voir le motif ***
        String username = authentication.getName();
        Utilisateur utilisateur = utilisateurService.findByUsername(username)
                .orElseThrow(() -> new RuntimeException("Utilisateur non trouvé"));
        
        Role userRole = utilisateur.getRole();
        boolean isAdminOrRH = (userRole == Role.ADMIN || userRole == Role.RH);
        boolean isOwnConge = utilisateur.getEmploye() != null && 
                             utilisateur.getEmploye().getId().equals(conge.get().getEmploye().getId());
        
        // Seuls ADMIN, RH ou le propriétaire du congé peuvent voir le motif
        boolean canViewMotif = isAdminOrRH || isOwnConge;
        
        model.addAttribute("conge", conge.get());
        model.addAttribute("canViewMotif", canViewMotif);
